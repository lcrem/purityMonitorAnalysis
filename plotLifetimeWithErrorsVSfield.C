...
void plotLifetimeWithErrorsVSfield(){
  ...
  gpurity1000->SetTitle("Lifetime; ;Lifetime [us]");
  gpurity1000->GetHistogram()->GetXaxis()->Set(numFields, -0.5, numFields-0.5);
  gpurity1000->GetHistogram()->GetYaxis()->SetTitle("Lifetime [us]");
  
  for (int i=1;i<=numFields;i++) gpurity1000->GetHistogram()->GetXaxis()->SetBinLabel(i,fieldNice[i-1].c_str());
  ...
  gpurity200->SetMarkerSize(1);
  gpurity200->SetMarkerColor(kBlue);
  gpurity200->SetLineColor(kBlue);
  gpurity200->Draw("pel");
  
  gpurity100->SetMarkerStyle(kCircle);
  gpurity100->SetMarkerSize(1);
  gpurity100->SetMarkerColor(kRed);
